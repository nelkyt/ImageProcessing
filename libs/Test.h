...
namespace MereTDD
{
    ...
    {
    public:
        MissingException(std::string exType) : mExType(exType) {}
    ...
    inline void confirm(std::string const expected, std::string const actual, const std::source_location location = std::source_location::current())
    {
        if (actual != expected)
        {
    ...
    std::string vector_to_string(const std::vector<T> vector)
    {
        ...
        {
            stringStream << static_cast<int>(vector[i]);
            if (i != vector.size() - 1)
            {
                stringStream << ", ";
            }
    ...
    template <typename T>
    inline void confirm(std::vector<T> expected, std::vector<T> actual, const std::source_location location = std::source_location::current())
    {
        if (actual != expected)
        {
            throw ActualConfirmException(vector_to_string(expected), vector_to_string(actual), location.line());
        }
    }
    ...
    inline void runTest(std::ostream &output, Test *test, int &numPassed, int &numFailed, int &numMissedFailed)
    {
        ...
        if (test->passed())
        {
            ...
            {
                // This test passed but was supposed to fail
                ++numMissedFailed;
